...
def get_positions_list(cursor):
    ...
    data = cursor.fetchall()
    positionlist = []
    fullpositionslist = [position for tuple in data for position in tuple]
    for position in fullpositionslist:
        positionindex = fullpositionslist.index(position)
        if defense_fixer(position).rstrip('0123456789 ').upper() not in positionlist:
            positionlist.append(defense_fixer(position).rstrip('0123456789 ').upper())
        else:
            pass
